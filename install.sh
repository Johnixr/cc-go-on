...
install() {
    ...
    if [[ -d "$HOME/.claude" ]]; then
        local skill_dir="$HOME/.claude/skills/share"
        ...
        log_info "  Claude Code: installed /share skill"
        installed_any=true
    fi

    # Codex
    if command -v codex &>/dev/null || [[ -d "$HOME/.codex" ]]; then
        log_info "  Codex: detected (adapter coming soon — contributions welcome!)"
        installed_any=true
    fi

    # Cursor
    if [[ -d "$HOME/.cursor" ]] || [[ -d "$HOME/Library/Application Support/Cursor" ]]; then
        log_info "  Cursor: detected (adapter coming soon — contributions welcome!)"
        installed_any=true
    fi

    if [[ "$installed_any" == false ]]; then
        log_warn "No supported AI tools detected"
        log_warn "You can still use share.sh directly from the command line"
    fi
    ...
    echo "    ~/.cc-go-on/share.sh export -p <passphrase>"
    echo "    ~/.cc-go-on/share.sh import <token> -p <passphrase>"
    echo ""
    if [[ -d "$HOME/.claude" ]]; then
        echo -e "  ${CYAN}Claude Code:${NC}"
        echo "    /share            Export current session"
        echo "    /share <token>    Import a shared session"
        echo ""
    fi
